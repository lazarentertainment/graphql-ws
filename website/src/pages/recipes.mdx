...
## [ws](https://github.com/websockets/ws) server usage with async iterator error conversion to `ExecutionResult`

Converts an exception thrown from an async iterator to an `ExecutionResult` and completes the stream.

```typescript
import { WebSocketServer } from 'ws';
import { isAsyncIterable } from 'graphql-ws/lib/utils';
import { useServer } from 'graphql-ws/lib/use/ws';
import { schema } from './my-graphql';
...
useServer(
  {
    schema,
    onOperation: (_ctx, _msg, _args, operationResult) => {
...
## [ws](https://github.com/websockets/ws) server usage with async iterator error conversion to `Error` message

Converts an exception thrown from an async iterator to an `Error` message, completing the stream.
...
useServer(
  {
    schema,
    onOperationFailure: (_ctx, _msg, _args, error) => [
